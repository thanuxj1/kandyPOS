...
import { Lock, Save, Trash2, PlusCircle, CheckCircle, ArrowLeft, Settings, Users, FolderKanban, Share2, Pencil, X } from "lucide-react"
...
export default function AdminDashboard() {
  ...
  useEffect(() => {
    const savedProjects = localStorage.getItem("kandy_projects")
    if (savedProjects) setProjects(JSON.parse(savedProjects))

    const savedTestimonials = localStorage.getItem("kandy_testimonials_v2")
    if (savedTestimonials) setTestimonials(JSON.parse(savedTestimonials))

    const savedPath = localStorage.getItem("kandy_admin_path") || "admin"
    setAdminPath(savedPath)
    ...
    setFacebookUrl(savedFacebook)
    setWhatsappUrl(savedWhatsapp)
  ...
  const handleAddProject = (e: React.FormEvent) => {
    ...
    const categoryLower = newProject.category.toLowerCase()
    const titleLower = newProject.title.toLowerCase()
    
    let autoIcon: "cafe" | "market" | "retail" = "retail"
    if (
      categoryLower.includes("cafe") || categoryLower.includes("dining") || categoryLower.includes("restaurant") || categoryLower.includes("bistro") || categoryLower.includes("food") ||
      titleLower.includes("cafe") || titleLower.includes("dining") || titleLower.includes("restaurant") || titleLower.includes("bistro") || titleLower.includes("food")
    ) {
      autoIcon = "cafe"
    } else if (
      categoryLower.includes("market") || categoryLower.includes("grocer") || categoryLower.includes("supermarket") || categoryLower.includes("store") ||
      titleLower.includes("market") || titleLower.includes("grocer") || titleLower.includes("supermarket") || titleLower.includes("store")
    ) {
      autoIcon = "market"
    }

    const autoImage = autoIcon === "cafe"
      ? "https://images.unsplash.com/photo-1554118811-1e0d58224f24?auto=format&fit=crop&w=600&q=80"
      : autoIcon === "market"
      ? "https://images.unsplash.com/photo-1542838132-92c53300491e?auto=format&fit=crop&w=600&q=80"
      : "https://images.unsplash.com/photo-1441986300917-64674bd600d8?auto=format&fit=crop&w=600&q=80"

    const projectImage = newProject.image.trim() || autoImage
    ...
    if (editingProject) {
      ...
      setProjects(updated)
      localStorage.setItem("kandy_projects", JSON.stringify(updated))
  ...
  const handleAddTestimonial = (e: React.FormEvent) => {
    ...
    if (editingTestimonial) {
      ...
      const updated = testimonials.map(t => {
        if (t.id === editingTestimonial.id) {
          return {
            ...
            company: "", // company empty, combined Role & Business saved under role
            text: newTestimonial.text,
            rating: 5,
            avatar: t.avatar || `/professional-headshot-${Math.floor(Math.random() * 5) + 1}.png`
          }
        }
        return t
      })
      setTestimonials(updated)
      localStorage.setItem("kandy_testimonials_v2", JSON.stringify(updated))
      ...
      const item: Testimonial = {
        ...
        text: newTestimonial.text,
        rating: 5,
        avatar: `/professional-headshot-${Math.floor(Math.random() * 5) + 1}.png`
      }

      const updated = [...testimonials, item]
      setTestimonials(updated)
      localStorage.setItem("kandy_testimonials_v2", JSON.stringify(updated))
    }
  ...
  // 3. Admin Settings (Dynamic Path Customization)
  const handleSavePath = (e: React.FormEvent) => {
    e.preventDefault()
    const cleanPath = adminPath.replace(/\//g, "").trim().toLowerCase()
    if (!cleanPath) return

    localStorage.setItem("kandy_admin_path", cleanPath)
    setSettingsSaved(true)
    setTimeout(() => setSettingsSaved(false), 3000)
  }

  // 4. Save Social & Contact Links
  const handleSaveSocials = (e: React.FormEvent) => {
    e.preventDefault()
    localStorage.setItem("kandy_social_facebook", facebookUrl.trim())
    localStorage.setItem("kandy_social_whatsapp", whatsappUrl.trim())
    localStorage.setItem("kandy_social_email", emailAddress.trim())
        ...
        <div className="w-full max-w-md p-8 rounded-3xl bg-[#0a0a0a]/80 border border-zinc-850 shadow-2xl relative z-10">
          ...
          <form onSubmit={handleLogin} className="space-y-4">
            <div>
              ...
              />
              {authError && (
                <div className="text-[10px] text-rose-500 font-mono text-center mt-2">
                  {authError}
                </div>
              )}
            </div>
      ...
      <div className="max-w-5xl mx-auto space-y-8 relative z-10">
            ...
            <div className="lg:col-span-3 space-y-4">
              ...
              <div className="space-y-3">
                {projects.map((proj) => (
                  <div
                    key={proj.id}
                    className="p-5 rounded-2xl bg-zinc-950/80 border border-zinc-850 flex justify-between items-start gap-4"
                  >
                    ...
                    <div className="flex items-center gap-2">
                      <button
                        ...
                        className="p-2 rounded-lg bg-zinc-900 border border-zinc-800 text-zinc-500 hover:text-cyan-400 hover:border-cyan-950 transition-colors"
                      >
                        <Pencil className="w-4 h-4" />
                      </button>
                      <button
                        onClick={() => handleDeleteProject(proj.id)}
                        className="p-2 rounded-lg bg-zinc-900 border border-zinc-800 text-zinc-500 hover:text-rose-500 hover:border-rose-950 transition-colors"
                      >
              ...
              <div className="flex items-center justify-between text-white font-bold text-sm border-b border-zinc-900 pb-2">
                ...
                {editingProject && (
                  <button 
                    ...
                    className="p-1 rounded bg-zinc-900 hover:bg-zinc-800 text-zinc-500 hover:text-zinc-300"
                  >
              ...
              <form onSubmit={handleAddProject} className="space-y-4 text-xs font-semibold">
                <div>
                  ...
                  <input
                    ...
                    placeholder="e.g. Golden Diner POS Integration"
                    className="w-full px-3.5 py-2.5 rounded-xl bg-zinc-900/50 border border-zinc-850 text-white placeholder-zinc-700 focus:outline-none focus:border-cyan-500 select-text"
                  />
                </div>
                ...
                <div>
                  ...
                  <input
                    ...
                    placeholder="e.g. Restaurant System"
                    className="w-full px-3.5 py-2.5 rounded-xl bg-zinc-900/50 border border-zinc-850 text-white placeholder-zinc-700 focus:outline-none focus:border-cyan-500 select-text"
                  />
                </div>
                ...
                <div>
                  ...
                  <textarea
                    ...
                    placeholder="Describe hardware/software setups..."
                    className="w-full px-3.5 py-2.5 rounded-xl bg-zinc-900/50 border border-zinc-850 text-white placeholder-zinc-700 focus:outline-none focus:border-cyan-500 resize-none select-text"
                  />
                </div>
                ...
                <div className="flex items-center gap-2.5 py-1">
                  <input
                    ...
                    checked={newProject.requiresPermission}
                    onChange={(e) => setNewProject({ ...newProject, requiresPermission: e.target.checked })}
                    className="w-4 h-4 rounded border-zinc-800 bg-zinc-900 text-cyan-500 focus:ring-cyan-500 focus:ring-opacity-25 accent-cyan-500 cursor-pointer"
                  />
                  <label htmlFor="requiresPermission" className="text-zinc-300 font-medium cursor-pointer">
                    Requires Permission Access Code
                  </label>
                </div>
                ...
                <div className="flex items-center gap-2 pt-2">
                  <Button
                    type="submit"
                    className="flex-1 bg-cyan-500 hover:bg-cyan-600 text-black font-bold py-5 rounded-xl transition-all"
                  >
                  ...
                  {editingProject && (
                    <Button
                      type="button"
                      onClick={handleCancelEdit}
                      className="bg-zinc-900 hover:bg-zinc-800 text-zinc-400 hover:text-white font-bold py-5 rounded-xl border border-zinc-800"
                    >
                      Cancel
                    </Button>
                  )}
        ...
        {activeTab === "testimonials" && (
          <div className="grid grid-cols-1 lg:grid-cols-5 gap-8">
              ...
              <div className="space-y-3">
                {testimonials.map((test) => (
                  ...
                  >
                    ...
                    <div className="flex items-center gap-2">
                      <button
                        onClick={() => handleEditTestimonialStart(test)}
                        className="p-2 rounded-lg bg-zinc-900 border border-zinc-850 text-zinc-500 hover:text-cyan-400 hover:border-cyan-950 transition-colors animate-fade-in"
                      >
                        <Pencil className="w-4 h-4" />
                      </button>
                      <button
                        onClick={() => handleDeleteTestimonial(test.id)}
                        className="p-2 rounded-lg bg-zinc-900 border border-zinc-850 text-zinc-500 hover:text-rose-500 hover:border-rose-950 transition-colors animate-fade-in"
                      >
                        <Trash2 className="w-4 h-4" />
                      </button>
                    </div>
            ...
            <div className="lg:col-span-2 p-6 rounded-2xl bg-zinc-950/40 border border-zinc-850 space-y-4 animate-slide-up">
              <div className="flex items-center justify-between text-white font-bold text-sm border-b border-zinc-900 pb-2">
                <div className="flex items-center gap-1.5">
                  {editingTestimonial ? (
                    <>
              ...
              <form onSubmit={handleAddTestimonial} className="space-y-4 text-xs font-semibold">
                <div>
                  ...
                  <input
                    ...
                    placeholder="e.g. Thanuja Senanayake"
                    className="w-full px-3.5 py-2.5 rounded-xl bg-zinc-900/50 border border-zinc-850 text-white placeholder-zinc-700 focus:outline-none focus:border-green-500"
                  />
                </div>
                ...
                <div>
                  ...
                  <input
                    ...
                    placeholder="e.g. Managing Director, The Royal Bean Cafe"
                    className="w-full px-3.5 py-2.5 rounded-xl bg-zinc-900/50 border border-zinc-850 text-white placeholder-zinc-700 focus:outline-none focus:border-green-500"
                  />
                </div>
                ...
                <div>
                  ...
                  <textarea
                    ...
                    placeholder="Enter what the business owner says..."
                    className="w-full px-3.5 py-2.5 rounded-xl bg-zinc-900/50 border border-zinc-850 text-white placeholder-zinc-700 focus:outline-none focus:border-green-500 resize-none"
                  />
                </div>
                ...
                <div className="flex items-center gap-2 pt-2">
                  <Button
                    type="submit"
                    className="flex-1 bg-green-500 hover:bg-green-600 text-black font-bold py-5 rounded-xl transition-all"
                  >
          ...
          <div className="max-w-xl mx-auto space-y-8">
            <div className="p-6 sm:p-8 rounded-3xl bg-zinc-950/80 border border-zinc-850 shadow-2xl space-y-6">
              ...
              <p className="text-zinc-400 text-xs sm:text-sm leading-relaxed">
                For ultimate security and dynamic flexibility, Kandy POS website allows you to **change the URL path** of this Admin panel dynamically.
              </p>
              ...
              <div className="p-4 rounded-2xl bg-cyan-950/10 border border-cyan-500/10 text-cyan-400/90 text-xs font-medium space-y-1">
                ...
                <div>• Current Live URL: <span className="font-mono text-white">http://localhost:3000/{adminPath}</span></div>
              </div>
                ...
                <div>
                  ...
                  <div className="flex rounded-xl bg-zinc-900 border border-zinc-850 overflow-hidden focus-within:border-cyan-500 transition-colors">
                    ...
                    <input
                      ...
                      placeholder="e.g. kandy-portal-dashboard"
                      className="flex-1 px-4 py-3.5 bg-transparent text-white placeholder-zinc-700 text-xs sm:text-sm font-semibold focus:outline-none select-text"
                    />
                  </div>
                  <p className="text-[10px] text-zinc-500 font-mono mt-2 leading-normal">
                    * Must be alphanumeric characters (no slashes, e.g. "kandy-portal").
                  </p>
                </div>

                <Button
                  type="submit"
                  className="w-full bg-gradient-to-r from-cyan-500 to-green-500 text-black hover:opacity-90 font-bold py-6 rounded-xl flex items-center justify-center gap-1.5 text-xs sm:text-sm"
                >
                  {settingsSaved ? (
                    <>
            ...
            <div className="p-6 sm:p-8 rounded-3xl bg-zinc-950/80 border border-zinc-850 shadow-2xl space-y-6">
              ...
              <p className="text-zinc-400 text-xs sm:text-sm leading-relaxed">
                Configure your official public contact links. These links are updated dynamically in the footer of the Kandy POS website.
              </p>
              ...
              <form onSubmit={handleSaveSocials} className="space-y-4">
                <div>
                  ...
                  <input
                    ...
                    className="w-full px-4 py-3 rounded-xl bg-zinc-900 border border-zinc-850 text-white placeholder-zinc-700 text-xs sm:text-sm font-semibold focus:outline-none focus:border-green-500 transition-colors select-text"
                  />
                </div>
                ...
                <div>
                  ...
                  <input
                    ...
                    placeholder="e.g. 94770000000 or https://wa.me/..."
                    className="w-full px-4 py-3 rounded-xl bg-zinc-900 border border-zinc-850 text-white placeholder-zinc-700 text-xs sm:text-sm font-semibold focus:outline-none focus:border-green-500 transition-colors select-text"
                  />
                  <p className="text-[10px] text-zinc-500 font-mono mt-1">
                    Enter your international format number (no + or spaces) or direct wa.me link.
                  </p>
                </div>
